...
static void write_cstr(int fd, const char *s)
{
    ...
    (void)chaseros_syscall_write(fd, s, n);
}
...
static void write_u64_dec(int fd, unsigned long long v)
{
    ...
    int i = 0;
    if (v == 0) {
        (void)chaseros_syscall_write(fd, "0", 1);
        return;
    }
    ...
    while (i > 0) {
        char c = buf[--i];
        (void)chaseros_syscall_write(fd, &c, 1);
    }
}
...
{
    if (v < 0) {
        (void)chaseros_syscall_write(fd, "-", 1);
...
void _start(void)
{
    ...
    write_cstr(1, "pid=");
    write_u64_dec(1, (unsigned long long)chaseros_syscall_getpid());
    write_cstr(1, " uptime_ticks=");
    write_u64_dec(1, (unsigned long long)chaseros_syscall_uptime_ticks());
    write_cstr(1, "\n");

    /* 阶段 3：IPC_CALL 一次 ping/pong（PID=3 服务） */
    {
        chaseros_message_t req;
        chaseros_message_t rep;
        req.sender = 0;
        req.type = CHASEROS_MSG_PING;
        for (size_t i = 0; i < CHASEROS_MAX_MSG_PAYLOAD; i++) {
            req.payload[i] = 0;
        }
        req.payload[0] = (uint8_t)'x';
        long cr = chaseros_syscall_ipc_call(CHASEROS_HYBRID_SERVICE_PID, &req, &rep);
    ...
    long n = chaseros_syscall_read(0, scratch, sizeof(scratch));
